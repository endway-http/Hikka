...
class Fun(loader.Module):
    ...
    strings = {
        ...
        "hackpenis_process": "<emoji document_id=6334357625160861194>💻</emoji> <b>Взлом Пениса в процессе... {}%</b>",
        "hackpenisedu": "<emoji document_id=5854762571659218443>✅</emoji> <b>Пенис успешно взломан!</b>",
    }
    ...
    @loader.command()
    async def hackpenis(self, message):
        """Взлом пениса"""
        perc = 0
 
        while(perc < 100):
                await utils.answer(message, self.strings["hackpenis_process"].format(perc))
                perc += random.randint(1, 3)
                await asyncio.sleep(0.3)
        
        await utils.answer(message, self.strings["hackpenisedu"])
